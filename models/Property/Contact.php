...
class Property_Contact extends Model
{
    ...
    private $_field = "contact_id, contact_name, contact_mobile, contact_phone";
    private $_prefixField = 'contact_';
    ...
    public function find($options=array())
    {
        ...
        if( !empty($options['q']) ){
            $condition .= !empty($condition) ? ' AND ':'';
            $condition .= "(contact_name LIKE '%{$options['q']}' OR contact_phone=:qPhone OR contact_phone=:qPhone1 OR contact_mobile=:qPhone OR contact_mobile=:qPhone1)";
            
            // contact_id=:id OR  $params[':id'] = intval($options['q']);
