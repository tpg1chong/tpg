...
(function( $, window, document, undefined ) {
	...
	$.fn.starsRatable.options = {
		rating: 0,
		level: { 
			1: { rating: 1, text: 'à¹à¸¢à¹ˆ' },
			2: { rating: 2, text: 'à¸žà¸­à¹ƒà¸Šà¹‰' },
			3: { rating: 3, text: 'à¸”à¸µ' },
			4: { rating: 4, text: 'à¸”à¸µà¸¡à¸²à¸' },
			5: { rating: 5, text: 'à¸¢à¸­à¸”à¹€à¸¢à¸µà¹ˆà¸¢à¸¡' }
		},
		showlabel: false
	};
	...
	var Selectbox = {
		...
		setMenu: function(){
			...
			if( self.options.add_item_url ){
				var $input = $('<input>', {class: 'inputtext', autocomplete:"off", placeholder: 'à¹€à¸žà¸´à¹ˆà¸¡à¹ƒà¸«à¸¡à¹ˆ...'});
				var $btn = $('<a />', {class: 'btn rfloat', 'text': 'à¸šà¸±à¸™à¸—à¸¶à¸'});
	...
	var Dropdown = {
		...
		setMenu: function () {
			...
			self.$model = $('<div/>', {class: 'uiTypeaheadView'});
        ...
        get: function(){
            ...
            /*if( self.options.pointer ){
           	...
           	if(overflow.Y == "Above"){
           		...
           		offset.top -= self.$elem.outerHeight() + 2;
           	}
	...
	var Editor_tags = {
		...
		setMenu: function () {
			...
			var $box = $('<div/>', {class: 'uiTypeaheadView'});
			self.$menu = $('<ul/>', {class: 'has-loading ', role: "listbox"});
		...
		getTag: function (text) {
			...
			var tag = $('<div>', {class: 'tag'}).append(
				$('<span>', {class: 'text'}).text( text ),
				$('<input>', {type: 'hidden', name: self.options.name, autocomplete: 'off', class: "hiddenInput" }).val( text ),
				$('<button>', {
					type: 'button',
					class: 'js-remove'
	            ...
	            image.onload = function() {
			        ...
			        $.ajax({
						...
						if( response.message ){
							...
							Event.showMsg({text: response.message, auto: true});
						}
    ...
    var Upload = {
		...
		setBox: function () {
			var self = this;
			var $wrapper = $('<div>', {class: 'upload-wrapper'});
			...
			ul.append( $('<li>', {class: 'rfloat' }).append( $('<a>', {class: 'mtm mrm btn btn-blue js-add', text: 'Upload Images'}) ) );
			...
			/**/
			/* meta */
			var $meta = $('<div>', {class: 'meta clearfix'});
			$actions = $('<ul>', {class: 'group-actions clearfix hidden_elem'});
			$actions.append( $('<li>', {class: 'delete' }).append( $('<button>', {class: 'btn', type: 'button', text: 'Delete'}) ) );

			$meta.append( $actions, self.options.message=='' ? '': $('<div>', {class: 'upload-message fsm pam uiBoxYellow mas'}).text( self.options.message ) );
			$header.append($tabs, $meta);

			var $body = $('<div>', {class: 'upload-wrapper-body'});
				...
				$sidebar.append( 
					$('<div>', {class: 'sidebar-content'}).append(
						$('<ul>').append(
							...
							, $('<li>').append( $('<a>').text( 'My Media' ) )
						)

					),
					$('<div>', {class: 'sidebar-footer'}).append(
						$('<ul>').append(
			...
			if( self.options.footer ){
				...
				var $footer = $('<div>', {class: 'upload-wrapper-footer clearfix'});
				$footer.append(
					$('<div>', {class: 'lfloat upload-status hidden_elem'}).append(
						  $('<i>', {class: 'icon-check mrs'})
						, $('<div>', {class: 'title'}).append(
							  'Uploaded&nbsp;'
							, $('<span>', {class: 'numbers'}).text( '(9/9 Files)' )
						)
						, $('<div>', {class: 'progress-bar medium'}).append(
							  $('<span>', {class: 'progress blue'})
						)
						, $('<div>', {class: 'fails'}).append( 'fails' )
					),
					$('<div>', {class: 'rfloat'}).append(
						$('<a>', {class: 'btn btn-blue'}).text( 'Done' )
					)
				);
		...
		editablePhoto: function(){
			var self = this;
			return $('<div/>', {class: 'uiEditablePhoto has-loading'}).css({
				width: self.options.max_width,
				height: self.options.max_height,
		...
		editablePhotoControls: function(){
			...
			return $('<div/>').addClass('controls').append( 
				...
				$('<a/>', {class: 'control remove'}).html(
					$('<i/>', {class: 'icon-remove', title: 'à¸¢à¸à¹€à¸¥à¸´à¸'})
				), 
				$('<a/>', {class: 'control checked'}).html(
					$('<i/>', {class: 'icon-check', title: 'à¹€à¸¥à¸·à¸­à¸'})
				)
			);
		},
    ...
    var ImageCover2 = {
		...
		displayImage: function() {
			...
			$image = $('<img>', {class: 'img img-crop', src: self.options.image_url, alt: ''});
			$img.html( $image );
		...
		setControl: function () {
			...
			var $edit = $('<div/>',{ class:'image-cover-edit', text: 'à¹€à¸›à¸¥à¸µà¹ˆà¸¢à¸™à¸£à¸¹à¸›'});
			var $del = $('<div/>',{ class:'image-cover-edit', text: 'à¸¥à¸š'});
	...
	var _update = {
		init: function(options, elem) {
			...
			self.$elem.change( function () {
				...
				Event.showMsg({text: 'à¸šà¸±à¸™à¸—à¸¶à¸à¹à¸¥à¹‰à¸§', load: true, auto: true});
		...
		setElem: function () {
			...
			var $table = $('<table/>', {class: 'calendarCloseDateGridTable'}).append( 
				...
				, $('<tr>').append( $('<td>', {colspan: 3, class: 'tar ptm'}).append(
					  self.$preveiw
					, $('<a>', {class: 'btn btn-cancel', text: 'à¸¢à¸à¹€à¸¥à¸´à¸'})
					, $('<a>', {class: 'btn btn-blue btn-submit', text: 'à¸™à¸³à¹„à¸›à¹ƒà¸Šà¹‰'})
				) )
			);
		...
		init: function (options,elem) {
			...
			self.$elem.delegate('select.labelselect', 'change', function () {
				...
				if( $(this).val()=='custom' ){
					...
					var $input = $('<input/>', {type: 'text', class: 'inputtext labelselect', name: $(this).attr('name')});
			...
			if( options.data ){
				...
				$.each( options.data, function (i, obj) {
					...
					if( !custom ){
						...
						var $wrap = $('<div>', {class: 'wrap'});
						var $input = $('<input/>', {type: 'text', class: 'inputtext labelselect', name: $select.attr('name')}).val( obj.name );
						$remove = $('<button/>', {type: 'button', class: 'icon-remove js-remove-label'});
	...
	var Timestamp = {
		init: function (options, elem) {
			...
			// '<span class="" data-time="'.$timestamp.'" title="'.$title.'">'.$text.'</span>'
		},
	};
	...
	$.fn.formcontacts = function( options ) {
		return this.each(function() {
			var $this = Object.create( FormContacts );
			$this.init( options,this );
			$.data( this, 'formcontacts', $this );
		});
	};
	...
	var Tabs = {
		init: function (options, elem) {
			...
			self.$elem.find('.tab-action [data-action]').click(function () {

				self.url = $(this).data('href');
				self.current = $(this).data('action');
				self.$content = self.$elem.find('.tab-content[data-content=' + self.current + ']');

				$(this).addClass('active').siblings().removeClass('active');

				if( self.$content.length==0 ) return false;
	...
	var RadioButtonGroup = {
		init: function (options, elem) {
			...
			self.$elem.find('.btn').click(function () {
				var $this = $(this);
	...
	var CheckBlock = {
		init: function (options, elem) {
			...
			self.$elem.find('.ui-item').click(function () {
				var $this = $(this);
	...
	$.fn.checkbox = function( options ) {
		return this.each(function() {
			var $this = Object.create( CheckBlock );
			$this.init( options,this );
			$.data( this, 'checkbox', $this );
		});
	};
	...
	var TimeSelector = {
		init: function(options, elem) {
			...
			self.options = $.extend( {}, $.fn.time_selector.options, options );

			self.$box = self.$elem.find('[role=list]');
			self.$input = self.$elem.find(':input');
			self.is_focus = false;
			self.is_hover = false;
			self.is__click = false;

			self.$input.focus(function(e) {
				...
				if( self.is_focus && !self.is_hover ){
					self.is_focus = false;
				}
			...
			self.$box.find('[data-value]').click(function () {
				$(this).addClass('active').siblings().removeClass('active');
				self.$input.val( $(this).attr('data-value') );
				...
				if( typeof self.options.onChange === 'function' ){
					...
					self.options.onChange.apply( self, arguments );
					// self.options.onChange( $(this).attr('data-value') );
				}
				// self.active(  );
			});
			...
			self.$input.keydown(function (e) {
				...
				if( $(this).val().lenght==self.options.maxlength ){
					self.is_focus = false;
					self.hide();
				}	
			});;
		...
		show: function () {
			...
			self.$box.find('[data-value='+ self.$input.val()+ ']').addClass('active').siblings().removeClass('active');
			self.$elem.addClass('active');
		},
		hide: function () {
			var self = this;
			...
			if( self.options.next ){
				setTimeout( function () {
					if( self.is__click /*&& $( self.options.next ).val() == ''*/ ){
	...
	$.fn.time_selector = function( options ) {
		return this.each(function() {
			var $this = Object.create( TimeSelector );
			$this.init( options, this );
			$.data( this, 'time_selector', $this );
		});
	};
	$.fn.time_selector.options = {
		share: 1,
		max: 2
	};
	...
	var inputSearchBox = {
		init: function ( settings, elem  ) {
			...
			// $el, settings, data, callback

			self.$elem = $(elem).parent();
			self.$elem.css('width', $(elem).outerWidth() );
			self.$overlay = $('<div>', {class: 'overlay'});

			self.$elem.addClass('ui-search').append( 
			...
			// set Data
			self.is_keycodes = [37,38,39,40,13];
			self.$input = self.$elem.find(':input[data-name=search]');
		...
		Events: function () {
			...
			self.$input.keyup(function (e) {
				...
				if( self.is_keycodes.indexOf( e.which )==-1 && !self.has_load ){
					...
					self.$input.parent().addClass('has-load');
					self.hide();

					clearTimeout( self.is_load );

					if(value==''){
						self.$input.parent().removeClass('has-load');
						return false;
					}

					self.is_load = setTimeout(function(argument) {
						self.has_load = true;
				...
				if( self.$menuCus ){
					if( keyCode==13 && self.$menuCus.find('li.selected').length==1 ){
						self.active(self.$menuCus.find('li.selected').data());
					}
				}
			}).click(function (e) {
				var value = $.trim($(this).val());
			...
			self.$elem.delegate('.js-cancel', 'click', function(event) {
				self.$elem.removeClass( 'active' );
				self.$input.val('').focus();
				self.$elem.find( '.overlay' ).empty();
			...
			self.$elem.delegate('.js-edit', 'click', function(event) {
		...
		search: function () {
			...
			$.ajax({
				...
				self.has_load = false;
				self.$elem.addClass('has-error').removeClass('has-load');
				
			}).always(function() {
				self.has_load = false;
				self.$elem.removeClass('has-load');
			});
		},
		setMenu: function () {
			var self = this;
		...
		buildFrag: function (results) {
			...
			$.each(results, function (i, obj) {
				...
				var li = $('<li/>');
				if( obj.image_url ){
					avatar = $('<div/>', {class:'avatar'}).html( $('<img/>', {calss: 'img', src: obj.image_url}) );
				...
				li.html( $('<a>').append( 
						  avatar
						, $('<span/>', {class: 'text', text: obj.fullname}) 
						// , $('<span/>', {class: 'subtext', text: obj.code})
						// , $('<span/>', {class: 'category', text: obj.phone_number})
					) 
				);
		...
		active: function ( data ) {
			...
			var $overlay = self.$elem.find( '.overlay' );
			...
			$overlay.append( $('<button>', {
				class: 'remove js-cancel',
				type: 'button',
			}).html( $('<i>', {class: 'icon-remove'}) ) );

			$overlay.append( $('<button>', {
				class: 'remove js-edit',
				type: 'button',
			...
			if( typeof self.settings.onChange === 'function' ) {
				self.settings.onChange( data, val );
			}
	...
	$.fn.input_search = function( options ) {
		return this.each(function() {
			var $this = Object.create( inputSearchBox );
			$this.init( options, this );
			$.data( this, 'input_search', $this );
		});
	};
	$.fn.input_search.options = {};
